...
def main():
    ...
    ap.add_argument('--out', default=os.path.join(HERE, 'REPORT_FINAL.md'))
    args = ap.parse_args()
    ...
    A('# Three-Broker AMQP Performance Benchmark — FINAL')
    A('')
    A('**HyrxMQ vs RabbitMQ vs LavinMQ — definitive run on one host, all three '
      'brokers in Docker, one compiled Go load generator.**')
    A('')
    ...
    A('- **LavinMQ large-payload collapse (important).** In `pubget` and '
      ...
      '`confirm|16384|1` 21 msg/s, `latency|262144|1` p50 41 ms), with stall/'
      'error flags in 11 cells. LavinMQ\'s overall geomean (6.38) is dominated '
    ...
    A('- **LavinMQ is genuinely fastest in `fanout`** (all 5 cells, ~1.19-1.24x '
      'over HyrxMQ): that is not a collapse artifact.')
    A('- **HyrxMQ had zero stalls and zero worker errors in all 85 cells / '
      '1,785 runs**; every stall/error above is LavinMQ.')
