...
| 4 | Per-destination clone (`router.mojo:161-181`) | **copied twice** per destination: `msg.payload()` → `Buffer.snapshot()` copies all bytes into a `BufferSnapshot` (`buffer.mojo:64`), then each byte is copied again into a fresh `Buffer` (`router.mojo:177-181`) | **rebuilt**: routing key copied; `message_id` replaced by `MessageID(0)`; headers replaced by an EMPTY dict → source metadata **discarded** (defect D1) | new `Message` per destination | 2 copies of every byte per destination | clone becomes immutable once enqueued | — |
...
|---|---|---|---|
| D1 | `src/hyrx/core/router.mojo:163-169` | Fan-out builds each destination envelope with `MessageID(0)` and an empty headers `Dict`: the published message's id and headers are **discarded**, silently. Payload is copied; metadata is lost. | Not observable through the core API — `Queue`/`Router` expose only `read_payload` and `read_routing_key`, no envelope read-back — so no test can pin the fix (tried: `Dict.__getitem__` copies and `Message`/`Queue` are move-only). AMQP content properties are separately declared NOT IMPLEMENTED (`src/hyrx/amqp/adapter.mojo:100`), so the loss is currently latent. Needs a spec + accessor package. |
| D2 | `router.mojo:170-181` + `buffer.mojo:64-73` | Payload is copied **twice** per destination (into a `BufferSnapshot`, then byte-by-byte into a `Buffer`) and once more per `read_payload`. Violates the Copy policy above ("every copy on a hot path must have a reason") with no instrumentation. | Removing the intermediate snapshot is a copy-path change; needs the measurement the doc demands first. |
