...
Physical measurement interfaces extracting observational states from physical environments.

---

## 1. Semantic Definition

**Sensors** is a first-class subdomain of SCR Physics (`SCR-LIB-PHYSICS`). It defines physical measurement interfaces extracting observational states from physical environments.
...
All operations within `Sensors` MUST adhere to the normative invariants of `SCR-LIB-PHYSICS`:
...
This document establishes the normative semantic meaning of `Sensors` in SCR Physics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
