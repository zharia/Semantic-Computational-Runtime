...
id: SCR-LIB-PHYSICS-MOMENTUM
name: Physics Momentum
...
# SCR Physics: Momentum

## Summary

Conserved linear (p = m*v) and angular (L = r x p) momentum of physical systems.

---

## 1. Semantic Definition

**Momentum** is a first-class subdomain of SCR Physics (`SCR-LIB-PHYSICS`). It defines conserved linear (p = m*v) and angular (L = r x p) momentum of physical systems.
...
All operations within `Momentum` MUST adhere to the normative invariants of `SCR-LIB-PHYSICS`:
...
This document establishes the normative semantic meaning of `Momentum` in SCR Physics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
