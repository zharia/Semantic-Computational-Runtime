...
Algorithmic computational methods resolving physical state equations under constraints.

---

## 1. Semantic Definition

**Solvers** is a first-class subdomain of SCR Physics (`SCR-LIB-PHYSICS`). It defines algorithmic computational methods resolving physical state equations under constraints.
...
All operations within `Solvers` MUST adhere to the normative invariants of `SCR-LIB-PHYSICS`:
...
This document establishes the normative semantic meaning of `Solvers` in SCR Physics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
