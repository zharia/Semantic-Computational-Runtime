...
Vector interaction causing linear momentum change, acceleration, or stress in physical bodies.

---

## 1. Semantic Definition

**Force** is a first-class subdomain of SCR Physics (`SCR-LIB-PHYSICS`). It defines vector interaction causing linear momentum change, acceleration, or stress in physical bodies.
...
All operations within `Force` MUST adhere to the normative invariants of `SCR-LIB-PHYSICS`:
...
This document establishes the normative semantic meaning of `Force` in SCR Physics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
