...
// Forward declare RenderContext to avoid OGRE dependency
...
// ─── Simulation Scene Interface (Renderer-Decoupled) ───────────────────────
...
public:
    ...
    /**
     * Attaches a rendering substrate. Called once when renderer is available.
     * Extracts OGRE pointers from RenderContext and initializes scene graph.
     */
    virtual void attachRenderer(RenderContext& ctx) = 0;

    /**
     * Detaches rendering substrate. Called once when renderer is being torn down.
     */
    virtual void detachRenderer(RenderContext& ctx) = 0;

    /**
     * Per-frame physics, kinematics, and simulation step.
     * Pure simulation — no OGRE/GPU pointer access.
     */
    virtual void update(
        float dt,
    ...
    /**
     * Renders simulation-specific presentation overlay.
     * Executes on main render thread only.
     */
    virtual void renderPresentation(
        RenderContext& ctx,
