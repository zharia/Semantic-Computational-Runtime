...
id: SCR-LIB-DYNAMICS-STATETRANSITION
name: Dynamics StateTransition
...
# SCR Dynamics: StateTransition

## Summary

Mapping, operator, or step advancing a dynamical system from an antecedent state to a successor state.

---

## 1. Semantic Definition

**StateTransition** is a first-class subdomain of SCR Dynamics (`SCR-LIB-DYNAMICS`). It defines mapping, operator, or step advancing a dynamical system from an antecedent state to a successor state.
...
All operations within `StateTransition` MUST adhere to the normative invariants of `SCR-LIB-DYNAMICS`:
...
This document establishes the normative semantic meaning of `StateTransition` in SCR Dynamics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
