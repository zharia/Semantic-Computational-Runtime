...
id: SCR-LIB-DYNAMICS-ORDINARYDIFFERENTIAL
name: Dynamics OrdinaryDifferential
...
# SCR Dynamics: OrdinaryDifferential

## Summary

Continuous systems where state depends on a single independent temporal variable.

---

## 1. Semantic Definition

**OrdinaryDifferential** is a first-class subdomain of SCR Dynamics (`SCR-LIB-DYNAMICS`). It defines continuous systems where state depends on a single independent temporal variable.
...
All operations within `OrdinaryDifferential` MUST adhere to the normative invariants of `SCR-LIB-DYNAMICS`:
...
This document establishes the normative semantic meaning of `OrdinaryDifferential` in SCR Dynamics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
