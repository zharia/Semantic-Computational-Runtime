...
id: SCR-LIB-DYNAMICS-PARTIALDIFFERENTIAL
name: Dynamics PartialDifferential
...
# SCR Dynamics: PartialDifferential

## Summary

Systems where state distributions depend on both spatial dimensions and time.

---

## 1. Semantic Definition

**PartialDifferential** is a first-class subdomain of SCR Dynamics (`SCR-LIB-DYNAMICS`). It defines systems where state distributions depend on both spatial dimensions and time.
...
All operations within `PartialDifferential` MUST adhere to the normative invariants of `SCR-LIB-DYNAMICS`:
...
This document establishes the normative semantic meaning of `PartialDifferential` in SCR Dynamics. Numerical engines, solvers, and simulation runtimes are subordinate to the contracts specified herein.
