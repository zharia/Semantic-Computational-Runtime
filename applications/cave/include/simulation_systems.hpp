#ifndef CAVE_SIMULATION_SYSTEMS_HPP
#define CAVE_SIMULATION_SYSTEMS_HPP
...
struct SystemContext {
    ...
    SystemContext(SubjectRegistry& sub, EventBus& eb, Ogre::SceneManager* sm = nullptr, Ogre::Camera* cam = nullptr, Ogre::RenderWindow* win = nullptr)
        : subjects(sub), events(eb), sceneMgr(sm), camera(cam), window(win) {}
};

/**
 * Abstract Base Subsystem interface.
 * Subsystems perform concurrent domain updates (updateAsync) and main-thread render syncs (renderSync).
 */
...
public:
    ...
    /**
     * Executes asynchronous domain simulation (physics, procedural computation, noise evaluation).
     * Can safely run concurrently across worker threads.
     */
    virtual void updateAsync(float dt, const UserInputState& input, SystemContext& ctx) {
        (void)dt; (void)input; (void)ctx;
    }

    /**
     * Executes main-thread GPU buffer updates, Ogre scene node transformations, and rendering commits.
     */
    virtual void renderSync(SystemContext& ctx, float dt) {
        (void)ctx; (void)dt;
    }

    virtual void handleEvent(const ISimulationEvent& event, SystemContext& ctx) {
        (void)event; (void)ctx;
    }

    virtual void cleanup(Ogre::SceneManager* scnMgr) {
        (void)scnMgr;
    }
};
...
public:
    ...
    virtual void updateAsync(float dt, const UserInputState& input, SystemContext& ctx) {
        for (auto& sub : sub_systems_) sub->updateAsync(dt, input, ctx);
    }

    virtual void renderSync(SystemContext& ctx, float dt) {
        for (auto& sub : sub_systems_) sub->renderSync(ctx, dt);
    }
    ...
    virtual void cleanup(Ogre::SceneManager* scnMgr) {
        for (auto& sub : sub_systems_) sub->cleanup(scnMgr);
    }
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            islandMesh = ctx.sceneMgr->createManualObject("OpenVdbIslandMeshObj");
            islandMesh->setDynamic(true);
            islandMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_MAIN);
            islandNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("OpenVdbIslandNode");
    ...
    void handleEvent(const ISimulationEvent& event, SystemContext& ctx) override {
        if (event.getEventType() == EventType::ISLAND_VOYAGE) {
            mesh_dirty = true;
            if (chunk_manager && ctx.sceneMgr) {
                chunk_manager->clearAllChunks(ctx.sceneMgr);
            }
        }
    }
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            if (ctx.sceneMgr->hasLight("CalderaGlowLight")) {
                ctx.sceneMgr->destroyLight("CalderaGlowLight");
            }
            if (ctx.sceneMgr->hasSceneNode("CalderaGlowNode")) {
                ctx.sceneMgr->destroySceneNode("CalderaGlowNode");
            }
            calderaGlow = ctx.sceneMgr->createLight("CalderaGlowLight");
            ...
            calderaGlowNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("CalderaGlowNode");
            calderaGlowNode->setPosition(160.0f, 62.0f, 160.0f);
            calderaGlowNode->attachObject(calderaGlow);

            if (ctx.sceneMgr->hasManualObject("LavaFlowMeshObj")) {
                ctx.sceneMgr->destroyManualObject("LavaFlowMeshObj");
            }
            if (ctx.sceneMgr->hasSceneNode("LavaFlowNode")) {
                ctx.sceneMgr->destroySceneNode("LavaFlowNode");
            }
            lavaMesh = ctx.sceneMgr->createManualObject("LavaFlowMeshObj");
            lavaMesh->setDynamic(true);
            lavaMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_6);
            lavaNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("LavaFlowNode");
            lavaNode->attachObject(lavaMesh);

            if (ctx.sceneMgr->hasManualObject("SmokePlumeMeshObj")) {
                ctx.sceneMgr->destroyManualObject("SmokePlumeMeshObj");
            }
            if (ctx.sceneMgr->hasSceneNode("SmokePlumeNode")) {
                ctx.sceneMgr->destroySceneNode("SmokePlumeNode");
            }
            smokeMesh = ctx.sceneMgr->createManualObject("SmokePlumeMeshObj");
            smokeMesh->setDynamic(true);
            smokeMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_8);
            smokeNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("SmokePlumeNode");
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            oceanMesh = ctx.sceneMgr->createManualObject("OceanMeshObj");
            oceanMesh->setDynamic(true);
            oceanMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_6);
            oceanNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("OceanNode");
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        ...
        if (ctx.sceneMgr) {
            ctx.sceneMgr->setAmbientLight(Ogre::ColourValue(0.32f, 0.38f, 0.46f));
            ctx.sceneMgr->setFog(Ogre::FOG_NONE);

            if (ctx.sceneMgr->hasLight("SunLight")) {
                ctx.sceneMgr->destroyLight("SunLight");
            }
            if (ctx.sceneMgr->hasSceneNode("SunLightNode")) {
                ctx.sceneMgr->destroySceneNode("SunLightNode");
            }
            sunLight = ctx.sceneMgr->createLight("SunLight");
            ...
            sunLightNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("SunLightNode");
            sunLightNode->setDirection(Ogre::Vector3(-0.4f, -1.0f, -0.6f).normalisedCopy());
            sunLightNode->attachObject(sunLight);

            if (ctx.sceneMgr->hasManualObject("SkyMeshObj")) {
                ctx.sceneMgr->destroyManualObject("SkyMeshObj");
            }
            if (ctx.sceneMgr->hasSceneNode("SkyNode")) {
                ctx.sceneMgr->destroySceneNode("SkyNode");
            }
            skyMesh = ctx.sceneMgr->createManualObject("SkyMeshObj");
            skyMesh->setDynamic(true);
            skyMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_SKIES_EARLY);
            skyNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("SkyNode");
    ...
    void renderSync(SystemContext& ctx, float dt) override {
        ...
        if (sky_system && skyMesh && player_sub && island_sub && island_sub->voxel_island) {
            ...
            if (is_underwater) {
                if (ctx.sceneMgr) {
                    ctx.sceneMgr->setFog(Ogre::FOG_EXP2, Ogre::ColourValue(0.02f, 0.18f, 0.32f), 0.040f);
                    ctx.sceneMgr->setAmbientLight(Ogre::ColourValue(0.12f, 0.45f, 0.60f));
                }
                if (ctx.window && ctx.window->getNumViewports() > 0) {
                    ctx.window->getViewport(0)->setBackgroundColour(Ogre::ColourValue(0.015f, 0.16f, 0.28f));
                }
                ...
                if (ctx.sceneMgr) {
                    ctx.sceneMgr->setFog(Ogre::FOG_NONE);
                    ctx.sceneMgr->setAmbientLight(sky_system->ambient_sky_color);
                }
                if (ctx.window && ctx.window->getNumViewports() > 0) {
                    ctx.window->getViewport(0)->setBackgroundColour(sky_system->zenith_color);
                }
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            parallaxMesh = ctx.sceneMgr->createManualObject("ParallaxMeshObj");
            parallaxMesh->setDynamic(true);
            parallaxMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_1);
            parallaxNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("ParallaxNode");
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            precipMesh = ctx.sceneMgr->createManualObject("PrecipitationMeshObj");
            precipMesh->setDynamic(true);
            precipMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_MAIN + 2);
            precipNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("PrecipitationNode");
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            vegMesh = ctx.sceneMgr->createManualObject("VegetationMeshObj");
            vegMesh->setDynamic(true);
            vegMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_MAIN);
            vegNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("VegetationNode");
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        if (ctx.sceneMgr) {
            boidMesh = ctx.sceneMgr->createManualObject("BoidMeshObj");
            boidMesh->setDynamic(true);
            boidMesh->setRenderQueueGroup(Ogre::RENDER_QUEUE_MAIN);
            boidNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("BoidNode");
            ...
            for (size_t i = 0; i < 6; ++i) {
                ...
                auto* light = ctx.sceneMgr->createLight(l_name);
                ...
                auto* node = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode(n_name);
    ...
    void renderSync(SystemContext& ctx, float dt) override {
        ...
        if (boid_system && boidMesh && island_sub && island_sub->voxel_island) {
            ...
            for (size_t i = 0; i < firefly_lights.size(); ++i) {
                firefly_lights[i]->setVisible(is_night);
                if (is_night && i < firefly_positions.size()) {
...
public:
    ...
    void renderSync(SystemContext& ctx, float dt) override {
        (void)dt;
        auto player_sub = ctx.subjects.getFirstSubjectOfType<PlayerSubject>(SubjectType::PLAYER);
        if (player_sub && ctx.camera) {
            auto* camNode = ctx.camera->getParentSceneNode();
            if (camNode) {
...
public:
    ...
    void initialize(SystemContext& ctx) override {
        ...
        if (ctx.sceneMgr) {
            ...
            waylandDisplay.initialize(ctx.sceneMgr, world_pos, look_target);
            waylandDisplay.is_visible = false;

            displayMesh = ctx.sceneMgr->createManualObject("InWorldDisplayMeshObj");
            displayMesh->setDynamic(true);
            displayNode = ctx.sceneMgr->getRootSceneNode()->createChildSceneNode("InWorldDisplayNode");
    ...
    void renderSync(SystemContext& ctx, float dt) override {
        ...
        if (displayMesh) {
            ...
            if (displayNode) {
                displayNode->setVisible(waylandDisplay.is_visible);
            }
        }
    }
...
public:
    ...
    void setSailForBiome(Island::IslandBiomeType biome, SystemContext& ctx) {
        auto island_sub = ctx.subjects.getFirstSubjectOfType<IslandSubject>(SubjectType::ISLAND);
        auto player_sub = ctx.subjects.getFirstSubjectOfType<PlayerSubject>(SubjectType::PLAYER);
...
public:
    ...
    void setOgreContext(Ogre::SceneManager* scnMgr, Ogre::Camera* cam, Ogre::RenderWindow* win) {
    ...
    void stepSimulation(float dt, const UserInputState& input) {
        ...
        for (auto& sys : systems_) {
            futures.push_back(std::async(std::launch::async, [this, sys, dt, input]() {
                sys->updateAsync(dt, input, context_);
            }));
        }
    ...
    void handleEvent(const ISimulationEvent& event) {
        for (auto& sys : systems_) {
            sys->handleEvent(event, context_);
        }
    }
...
#endif // CAVE_SIMULATION_SYSTEMS_HPP
